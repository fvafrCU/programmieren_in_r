...
Wenn ich nicht eine bestimmte Hilfeseite aufrufen will (vielleicht, weil ich
nicht wei&szlig;, welche ich suche), kann ich die Hilfeseiten auch
durchsuchen:
``` {r}
??Kolomogorov
``` 
Durchsucht die Hilfeseiten nach dem Wort "Kolomogorov", und verweist mich dann
(obwohl ich Kolmogorov falsch geschrieben habe)
auf die Hilfeseite der Funktion `ks.test` aus dem Package *stats*, die den
Kolmogorov-Smirnov-Test implementiert.
...
diese html-Seite wird beispielsweise durch das **R**-Package `knitr` gesetzt, 
...
Unter Windows kann `tempdir()` ein recht langwieriger Pfad sein, wenn Du den
windows explorer an einem solchen Pfad ge&ouml;ffnet haben m&ouml;chtest, kannst
Du das unter Windows mit `shell.exec()` tun:
``` {r, eval = FALSE}
shell.exec(not_so_temporary_directory)
``` 

Unter unixoiden Betriebssystemen hilft Dir 
`system2("/path/to/file_browser", not_so_temporary_directory)`.
...
``` {r}
<#include "src/f_personal_directories.r">
...
Argument erweitern:
``` {r}
<#include "src/f1_personal_directories.r">
...
[hier](hhttps://gitlab.com/fvafrcu/programmierleitfaden/raw/master/programmierleitfaden.ps)
...
<p align="right">
**Last modified:** 2018-07-20, 08:01
</p>

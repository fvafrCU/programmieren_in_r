...
author: Dominik Cullmann, Elke Erhardt, Sebastian Schmack
---
<!---
# use 
...
a_number <- 3
``` 
Das ist schon recht technisch, denn in vielen anderen Sprachen werden Objekte
...
M&ouml;chtest Du alle von Dir erstellten Objekte auflisten, hilft Dir
``` {r}
ls()
``` 


Wenn Du ein Objekt l&ouml;schen m&ouml;chtest, vielleicht weil Du inzwischen so
viele erstellt hast, dass Du den &Uuml;berblick verlierst, kannst Du das mit der
Funktion `remove` oder ihrer Kurzform `rm` tun:
``` {r}
rm(a_number)
``` 
Jetzt ist es weg:
...
ls()
``` 
M&ouml;chtest Du nur wissen, ob es ein bestimmtes Objekt gibt, kannst Du die 
Funktion 
`exists()` nutzen.


M&ouml;chtest Du alle von Dir erstellten Objekte l&ouml;schen, verwende die Form
`rm(list = ls())`. Das musst Du jetzt noch nicht verstehen, aber merke es
Dir bitte f&uuml;r sp&auml;ter.
...
### Was soll der Pfeil, geht nicht auch ein Gleichheitszeichen?
Das ist eine Frage, die ich immer wieder h&ouml;re. Ich beantworte sie daher 
hier, obwohl ich Dir [erst gleich](#funktionen) die Funktionen vorstellen
werde. 

**Die kurze Anwort lautet:**
    Ja, es geht auch ein Gleichheitszeichen, gew&ouml;hne Dir das aber nicht an.

**Die lange Anwort lautet:**
[Viele Programmiersprachen nutzen es als _den_ Zuweisungsoperator, viele aber auch nicht,](https://en.wikipedia.org/wiki/Assignment_(computer_science)#Notation) und das 
[nicht ohne Grund](https://en.wikipedia.org/wiki/Assignment_(computer_science)#Assignment_versus_equality).
In **R** geht auch
``` {r}
a_string = "This is a string."
print(a_string)
``` 
Guter Stil ist das aber nicht. Nicht nur, weil Du das aus Versehen mit einen 
Gleichheitsoperator verwechseln k&ouml;nntest, sondern vor allem  weil das 
Gleichheitszeichen f&uuml;r die Argumenten&uuml;bergabe in Funktionen benutzt 
wird, und 
``` {r, eval = FALSE}
...
``` 
daher nicht funktioniert (der output sieht bei Dir ein wenig anders aus, aber 
                          der Fehler ist der selbe).

Statt dessen funktioniert aber
``` {r, eval = FALSE}
print(another_string <- "This is a string.")
``` 
Und w&auml;hrend der &Uuml;bergabe des Funktionsargumentes `x`
(hier `another_string <- "This is a string."`) erfolgt auch gleich die Zuweisung
und wir sehen das neue Object:
``` {r}
ls()
``` 
Falls Du dem nicht gut folgen konntest: ist nicht schlimm. Aber falls Du Dich 
sp&auml;ter mal fragst, warum Du immer die Pfeile tippen sollst, kannst Du hier 
nochmal nachlesen. 

_Du kannst in fast jedem Editor keyboard shortcuts festlegen. In **RStudio** ist
der Zuweisungsoperator standarm&auml;&szlig;ig auf [Alt]+[-] gelegt. Du kannst 
den shortcut "Insert Assignment Operator" unter 
"Tools" > "Modify keyboard shortcuts..." &auml;ndern._ 



## Funktionen 
Funktionen habe wir ja schon benutzt: `print()` und `help()`
...
## Operatoren
**R** kennt die &uuml;blichen und ein paar andere arithmetische Operatoren, die 
Hilfe findest Du unter `help("Arithmetic", package = "base")`Arithmetic.
...
1. Von Nutzerinnen und Nutzern beigesteuerte Pakete, **R**-contributed genannt. 
   Sie werden ebenfalls auf den **R**-Servern verwaltet und sind sehr 
   unterschiedlicher Qualtit&auml;t. Es gibt 
...
Dort liest Du unter **Description**, dass die Kontrollstrukturen in **R** in 
etwa die 
selben sind wie in allen anderen Programmiersprachen der ALGOL-Familie, und 
unter **Usage** siehst Du, dass es sich nur um eine Handvoll Konstrukte 
handelt.
...
Ergebnis, entweder *You win!* oder *You lose!* ausgeben.
...
## Schleifen 
... f&uuml;hren Code wiederholt aus. Wenn Du einen Codeblock mehr als zweimal
untereinander kopierst und nur minimale &Auml;nderungen vornimmst, ist er in Wirklichkeit eine Schleife.
Es gibt gute Schleifen und schlechte
...
Dazu brauchen wir allerdings ein Objekt, hier `value`, dem wir die
Werte von i zuaddieren k&ouml;nnen. 
Da 0 neutral zur Addition ist, bef&uuml;llen wir `value` mit dem Wert 0.

Das "i gleich 1 bis n" hat die Form `i in seq(1, n)`, `seq()` ist eine Funktion, 
die Sequenzen, also Zahlenreihen, generiert. Die `for`-Schleife weist in jedem 
Durchlauf den aktuellen Wert auf der rechten Seite von `in` dem Objekt auf der 
linken Seite von `in` zu. Existiert dies Objekt (hier `i`) bereits, wird sein 
Inhalt &uuml;berschrieben, sonst wird es durch die Zuweisung angelegt. In beiden 
F&auml;llen existiert es nach Ablauf der Schleife und enth&auml;lte den letzten 
von der rechten Seite zugewiesenen Wert.
...
Schleifen), dann <a name="sum"></a>ist
``` {r}
a  <- c(2, 3, 4, 10)
value <- 0
...
Da sie dauernd auftritt, gibt es neben dieser (mathematischen Form) noch die
Kurzform f&uuml;r Programmierende:
``` {r}
a  <- c(2, 3, 4, 10)
value <- 0
for (a_i in a) {
...
anschliessende Klammern aufrufst (ich unterdr&uuml;cke hier den output, der ist
l&auml;nglich):
``` {r, results = "hide"}
apply
```
...
<div style="background-color:#FFE9E9; border:3px #C4DCFB solid;">
**Aufgabe:**
Schreibe ein Programm, dass Dir den ["hello, world"-Code](src/hello_world.r)
...
Bundesau&szlig;en-, Bundesinnen- und Bundesverteidigungsministerinnen und
-minister Deinen Kurs besucht haben, hast Du die Zeit allemal wieder raus.
</div>
...
bekommst, wo Du W&ouml;rter erwartest, k&ouml;nnte ein `factor` dahinterstecken:
...
Wenn Du einen Codeblock mehr als zweimal untereinander kopierst und nur minimale 
&Auml;nderungen vornimmst, ist er in Wirklichkeit eine Schleife.
Wenn Du einen Code-Block mehr als zweimal an unterschiedliche Stellen kopierst
und nur minimale &auml;nderungen vornimmst, solltest Du aus ihm eine Funktion 
basteln (in Rstudio kannst Du markierte Codezeilen über die 
...
Funktionsaufruf &uuml;berschreiben kannst:
``` {r}
create_personal_directories("files/names.txt", 
...
<div style="background-color:#FFE9E9; border:3px #C4DCFB solid;">
**Aufgabe:**
Schreibe eine Funktion, die die Element eines Vektors aufsummiert (diese
Funktion gibt es in **R** schon, und sie ist schneller und stabiler als die, die
wir schreiben werden, aber daf&uuml;r kannst Du [dieses Beispiel](#sum) einfach
anpassen), die also so funktioniert:
...
[Hier findest Du meine L&ouml;sung.](answers/sum.r)
</div>
...
Sie dient nur dazu, dass ich den Code nicht dreimal tippen muss. Das
hei&szlig;t: sie sollte eigentlich eine Schleife sein.
...
<p align="right">
**Last modified:** 2017-11-17, 09:06
</p>

...
Operatoren verkn&uuml;pfen:
``` {r}
a_number != 4 || a_number == 4
``` 
*(Es gibt in **R** auch Funktionen, die komplexere "Dinge" miteinander 
vergleichen, beispielsweise `?identical` und `?all.equal`)*
...
**R** sind Funktionsaufrufe. Eine Liste der in Deiner Umgebung verf&uuml;gbaren
...
Deinen Namen zu raten und meine Eingabe in einem Objekt speichert.
...
``` 
Das war die einfachste Variante, etwas ausgefeilter wird es so:
``` {r}
some_vector <- c("a", "b", "c")
...
findest Du unter http://10.33.50.43/R_User_Group/data_types.html.
...
<p align="right">
**Last modified:**                      2015-12-01, 10:45
</p>
